...
```bash
npm run run:bar:xlsx:all -- --open-output
```

Pie (XLSX):

```bash
npm run run:pie:xlsx:all -- --open-output
```

Scatter (XLSX):

```bash
npm run run:scatter:xlsx:all -- --open-output
```
...
- `output.format = "xlsx"`:
  - Fetches stored-procedure rows from `data_source`.
  - For single-result-set line/bar/pie/scatter charts, exporter can write into a template workbook with embedded chart.
...
## Excel Chart Templates (Embedded Charts)

Reusable Excel templates with embedded, editable charts are included at:

- `chart-contract/templates/line-series-template.xlsx`
- `chart-contract/templates/bar-grouped-template.xlsx`
- `chart-contract/templates/pie-template.xlsx`
- `chart-contract/templates/scatter-template.xlsx`

Template design:

- Sheet `Data` contains chart source rows.
- Sheet `TemplateMeta` explains required/optional columns.
- Embedded chart is anchored on the `Data` sheet.
- Editing values in the sheet updates the chart immediately in Excel.

Column mappings in `Data`:

- line: `A=date`, `B/C/D=series values`
- bar: `A=date`, `B/C/D=series values`
- pie: `A=label`, `B=value`
- scatter: `A=x`, `B/C/D=series values`

### Regenerate templates

All template workbooks are generated by:
...
Run with the local virtualenv Python:
...
```bash
npm run generate:templates:xlsx
```

### Use template-backed XLSX export

Set these fields in your request:

```json
"export_options": {
  "xlsx": {
    "template_path": "chart-contract/templates/<chart-type-template>.xlsx",
    "template_sheet_name": "Data",
    "result_set_indices": [0]
  }
}
```
...
- For line/bar series exports, tidy rows are pivoted to wide columns for the template chart.
- For pie exports, `categoryField` and `valueField` map to `label/value`.
- For scatter exports, `xField` and `yField` map to `x/Series`.
- Up to 3 series are mapped into template columns `B:D` where applicable.
- The generated workbook keeps an embedded Excel chart that updates when users edit data cells.
